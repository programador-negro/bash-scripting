...
instalar_postgres () {
    ...
    verifyInstall=$(which psgl)
    if [ $? -eq 0 ]; then
        echo -e "postgres ya se encuentra instalado en el equipo"
    else
        read -s -p "ingresar contraseña para sudo: " passsword
        read -s -p "ingresar contraseña para postgres :" passwordPostgres
